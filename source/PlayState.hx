...
class PlayState extends MusicBeatState
{
	...
	public var camFollowOffset:Array<Float> = [0, 0];
	public var camFollowPosOffset:Array<Float> = [0, 0];
	...
	public static var instance:PlayState; // haxe kinda sucks

	public var ratingsGroup:FlxSpriteGroup;

	public var musicWhenLeave:Bool = true; // whether to pl;ay msuic when you exit to menu or end song
	...
	override public function create()
	{
		...
		// for lua
		instance = this;
		FunkinLua.curInstance = instance;
		// CoolUtil.curLuaState = 'playstate';
		...
		// Ratings
		ratingsData.push(new Rating(scoreSpr.sick)); // default rating

		var rating:Rating = new Rating(scoreSpr.good);
		rating.ratingMod = 0.7;
		rating.score = 200;
		rating.noteSplash = false;
		ratingsData.push(rating);

		var rating:Rating = new Rating(scoreSpr.bad);
		rating.ratingMod = 0.4;
		rating.score = 100;
		rating.noteSplash = false;
		ratingsData.push(rating);

		var rating:Rating = new Rating(scoreSpr.shit);
		rating.ratingMod = 0;
		rating.score = 50;
		rating.noteSplash = false;
		ratingsData.push(rating);
		...
		ratingsGroup = new FlxSpriteGroup();
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			var strumYPos:Array<Float> = [];
			for (i in 0...opponentStrums.length)
			{
				setOnLuas('defaultOpponentStrumX' + i, opponentStrums.members[i].x);
				setOnLuas('defaultOpponentStrumY' + i, opponentStrums.members[i].y);
				// if(ClientPrefs.middleScroll) opponentStrums.members[i].visible = false;
				strumPos.push(opponentStrums.members[i].x);
				strumYPos.push(opponentStrums.members[i].y);
			}
			for (i in 0...playerStrums.length)
			{
				setOnLuas('defaultPlayerStrumX' + i, playerStrums.members[i].x);
				setOnLuas('defaultPlayerStrumY' + i, playerStrums.members[i].y);
				strumPos.push(playerStrums.members[i].x);
				strumYPos.push(playerStrums.members[i].y);
			}
			setOnLuas('defaultStrumPos', strumPos);
			setOnLuas('defaultStrumPosY', strumYPos);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (unspawnNotes.length > 0)
					...
					oldNote = null;
				var cool:NoteValuesOverride = cast callOnHscripts('onUnspawnNoteCreate', [songNotes, daNoteData, gottaHitNote, oldNote, i]);

				if (cool != null)
				{
					try
					{
						if (cool.songNotes != null)
							songNotes = cool.songNotes;
						if (cool.daNoteData != null)
							daNoteData = cool.daNoteData;
						if (cool.gottaHitNote != null)
							gottaHitNote = cool.gottaHitNote;
						if (cool.oldNote != null)
							oldNote = cool.oldNote;
					}
					catch (e:Dynamic)
					{
						lastHScriptReturn.error(e, 'Error overriding note values');
					}
				}
	...
	override public function update(elapsed:Float)
	{
		...
		if (!inCutscene)
		{
			...
			camFollowPos.setPosition(FlxMath.lerp(camFollowPos.x + camFollowPosOffset[0], camFollow.x + camFollowOffset[0], lerpVal),
				FlxMath.lerp(camFollowPos.y + camFollowPosOffset[1], camFollow.y + camFollowOffset[1], lerpVal));
			if (!startingSong
				&& !endingSong
				&& boyfriend.animation.curAnim != null
		...
		if (camZooming)
		{
			...
			camHUD.zoom = FlxMath.lerp(hudZoom, camHUD.zoom, CoolUtil.boundTo(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1));
		}
	...
	{
		Paths.image(scoreSpr.sick);
		Paths.image(scoreSpr.good);
		Paths.image(scoreSpr.bad);
		Paths.image(scoreSpr.shit);
		Paths.image(scoreSpr.combo);

		for (i in 0...10)
		{
			Paths.image(scoreSpr.num + i);
		}
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(scoreSpr.combo));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(scoreSpr.num + Std.int(i)));
...
} typedef NoteValuesOverride =
{
	var songNotes:Array<Dynamic>;
	var daNoteData:Null<Int>;
	var gottaHitNote:Null<Bool>;
	var oldNote:Null<Note>;
	var id:Null<Int>;
}
